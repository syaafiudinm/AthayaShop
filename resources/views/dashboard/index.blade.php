    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-[#C7EEFF] p-6 rounded-2xl shadow-sm flex items-center justify-between">
                        <div>
                            <p class="text-gray-800 font-bold text-sm mb-6">Total Transaksi</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-[#C7EEFF] p-6 rounded-2xl shadow-sm text-center flex flex-col justify-center">
                ...
                <p class="text-5xl font-bold text-gray-800 my-2">{{ $witaNow }}</p>
                <p class="text-gray-600">
                    <svg class="inline-block w-4 h-4 -mt-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm">
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-3 items-center text-sm text-gray-700 hover:bg-gray-50 p-2 rounded-lg">
                        ...
                        <div class="text-center">{{ $sale->created_at->format('l, d F Y') }}</div>
...
<script>
    // Opsi umum untuk semua donut chart agar tidak mengulang kode
    const commonChartOptions = {
        chart: {
            type: 'donut',
            width: 130, // Lebar chart
            height: 120, // Tinggi chart
        },
        plotOptions: {
            pie: {
                donut: {
                    size: '60%', // Ukuran lubang donat
                    labels: {
                        show: true,
                        name: {
                            show: false, // Sembunyikan label nama (e.g., "Alfa")
                        },
                        value: {
                            show: true, // Tampilkan angka di tengah
                            fontSize: '24px',
                            fontWeight: 'bold',
                            color: '#1f2937', // Warna teks angka (abu-abu gelap)
                            offsetY: 8,
                            formatter: function (val) {
                                return val; // Tampilkan nilai asli
                            }
                        },
                        total: {
                            show: false,
        ...
        stroke: {
            width: 0 // Hapus garis pinggir
        },
        legend: {
            show: false // Sembunyikan legenda
        },
        tooltip: {
            enabled: true // Matikan tooltip saat hover
        }
    };

    // 1. Chart untuk ALFA
    const optionsAlfa = {
        ...commonChartOptions, // Gabungkan dengan opsi umum
        series: [{{ $hadirCount }}],
        labels: ['Hadir'],
        colors: ['#3B82F6'], // Merah (red-500) 
    };
    const chartAlfa = new ApexCharts(document.querySelector("#chartAlfa"), optionsAlfa);
    chartAlfa.render();
    ...
    const optionsIzin = {
        ...
        series: [{{ $izinCount }}],
        labels: ['Izin'],
        colors: ['#1F2937'], // Abu-abu gelap (gray-800)
    };
